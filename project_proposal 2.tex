...
The tree systematically goes to potential matches (\texttt{Yes} path) based on the highest-priority attribute first through moving down the tree.
Each node will be added to a list \texttt{result} accordingly.
\\
\\
If there is more than one node with common attributes, go from the leftmost node. (For example, if A and B share the same attributes, A goes first.)
