...
def plot_user_connections(users: list[User], search_name: str = None, positions=None) -> go.Figure:
    ...
    # Add all nodes first, even if they don't have connections
    for user in users:
        G.add_node(user.name, size=max(1, user.social_degree))  # Use a minimum size of 1 for visibility
    
    # Add edges based on social_current connections
    for user in users:
...
# Generate the initial user list and graph with positions
